...
public class OpenSSHParser : ILogParser
{
    ...
    private static readonly Regex _openSSHRegex = new(@"^(?<ts>\w{3}\s+\d{1,2}\s\d{2}:\d{2}:\d{2})\s(?<host>\S+)\s(?<component>\w+)\[(?<pid>\d+)\]:\s(?<message>.+)$", RegexOptions.Compiled);
    ...
    {
        var match = _openSSHRegex.Match(line);
        if (!match.Success)
            return null;
